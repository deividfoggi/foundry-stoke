<!-- devsquad-template: .github/docs/coding-guidelines.md vunknown sha=ca2bae8e5651 -->
# Coding Guidelines
...
This repository is a **monorepo** hosting a single library, implemented independently per language under top-level folders. The public behavior must stay consistent across languages; the idioms must stay native to each language.

### Repository Layout

| Path | Purpose |
| --- | --- |
| `python/` | Python implementation and packaging (beta) |
| `dotnet/` | C# (.NET) implementation and packaging (beta) |
| `go/` | Reserved/deferred. Not part of the beta. Added only once an official Foundry SDK for Go exists (today Go has only the REST `/sessions` API). |
| `docs/` | Cross-language specs, ADRs, and templates |
| `.github/` | SDD Framework configuration and CI workflows |

Beta languages are **Python and .NET only**. The cross-language contract must stay designed so a Go implementation can be added later with no breaking changes.

Rules:

- Keep language implementations isolated. Do not share build tooling or source across language folders.
- Cross-cutting decisions (public API shape, provider interface contract, naming of concepts) are recorded once in an ADR under `docs/architecture/decisions/` and apply to all languages.
- The **durable store** is accessed through a pluggable provider interface (provider pattern) in every language. Core logic must not depend on a concrete store implementation.
- Prefer names and concepts drawn from the official Foundry documentation to maximize compatibility across SDKs.

### Language Conventions

- **Python (beta)**: target Python 3.10+ as supported by the official Foundry Python SDK (`azure-ai-projects`). Package with `pyproject.toml`. Format and lint with `ruff`; type-check with a static checker (e.g., `mypy` or `pyright`). Publish to PyPI. Prefer explicit result-based error handling; use exceptions for exceptional cases.
- **C# (.NET) (beta)**: target .NET 8 (LTS) as supported by the official Foundry .NET SDK (`Azure.AI.Projects`). Enable nullable reference types. Follow standard .NET naming conventions. Publish to NuGet. Use `async`/`await` where I/O is involved, following the async guidance above.
- **Go (deferred)**: reserved, not part of the beta. Add only once an official Foundry Go SDK exists (today Go has only the REST `/sessions` API). When added: use Go modules, format with `gofmt`, vet with `go vet`, idiomatic `error` returns, publish via the standard module proxy.

Each language is versioned and released independently (independent semver on PyPI and NuGet); a change does not force a lockstep version bump across languages.

### Cross-Language Consistency

- The public API surface (concepts, method names, provider interface members) must map recognizably across languages, adapted to each language's naming conventions.
- Behavioral changes to the library must be reflected in every implemented language before release, or the divergence must be recorded in an ADR.
